...
from devsecops_agent.service import analyze_scan_results, create_remediation_issues, post_analysis_to_pull_request
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--create-github-issues", action="store_true", help="Create GitHub issues for remediation tracking")
...
def main() -> None:
    ...
    if args.create_github_issues:
        if not all([args.github_owner, args.github_repo]):
            raise SystemExit("--github-owner and --github-repo are required when creating issues")
        from devsecops_agent.config import settings
        ...
        create_remediation_issues(args.github_owner, args.github_repo, token, findings, analyses)
